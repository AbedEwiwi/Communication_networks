...
import random # @see installNonNFToRRules and installNFToRRules
from helper import *
from parser import *
log = core.getLogger()
...
class Tutorial (object):
    ...
    def _handle_PacketIn (self, event):
            ...
            if arpPacket.opcode == arp.REPLY:
                ...
                for nf in services:
                    if services[nf][1] == str(ipSrc):
                        ...
                        elif services[nf][0] and services[nf][NF_DPID] != switch:
                            log.debug("NF " + str(nf) + " changed location from s" + services[nf][NF_DPID] + " to s" + switch + " port " + str(inPort))
                            services[nf][NF_DPID] = switch
                            services[nf][NF_PORT] = str(inPort)
        ...
        if packet.type != ethernet.LLDP_TYPE:
            ...
            if int(switch) in range(100,200):
                self.handle_PacketIn_CoreSwitch(packet,packet_in,src,dst,inPort,switch)
                return

            elif int(switch) in range(200, 300):
                self.handle_PacketIn_SpineSwitch(packet,packet_in,src,dst,inPort,switch)
                return

            else:
                self.handle_PacketIn_TorSwitch(packet,packet_in,src,dst,inPort,switch)

    def update_Host_Location(self,host,oldLoc,newLoc,newPort):
        """
        Updates the location of the host in case it changed its location. As part of the process,
        it deletes all rules installed on switches and reinstalls the rules.
        """

        log.debug("Host " + host + " changed location from: " + "[" + oldLoc + "]  to: [" + newLoc + ", " + str(newPort) + "]")
        hostsLocation[host][0] = newLoc
        hostsLocation[host][1] = newPort
        Tutorial.deleteAllRules()
        Tutorial.installPolicyRules()

    def handle_PacketIn_TorSwitch(self,packet,packet_in,src,dst,inPort,switch):
        """
        Defines the part of _handle_PacketIn() where the referring switch is a ToR switch.
        """

        for nf in services:
            if services[nf][0] == True and services[nf][NF_DPID] == switch and int(services[nf][NF_PORT]) == inPort:
                for host in hostsLocation:
                    if str(hostsLocation[host][0]) == switch and int(hostsLocation[host][1]) == inPort and str(host) == dst:
                        return
        ...
        elif hostsLocation.get(src) is not None:
            hostSwitch = hostsLocation.get(src)[0]
            if packet.type == ethernet.VLAN_TYPE:
                vlanPacket = packet.payload
                vlanTag = vlanPacket.id
                if vlanTag == VLAN_HOST :
                    # log.debug("entered the vlan_host section src is "+src +" and switch "+ switch +" with vlan_tag "+str(vlanTag))
                    if switch != hostSwitch:
                        self.update_Host_Location(src,hostSwitch,switch,inPort)
                    return 
            else:
                if switch != hostSwitch:
                    self.update_Host_Location(src,hostSwitch,switch,inPort)
        ...
        if hostsLocation.get(dst) is None:
            ...
            if packet.type == ethernet.VLAN_TYPE:
                vlanPacket = packet.payload
                vlanTag = vlanPacket.id
                if vlanTag == VLAN_DONE :
                    myNFPorts = self.getMyNfPorts(switch)
                    if int(dstPort) in myNFPorts:
                        for host in hostsLocation:
                            if str(hostsLocation[host][0]) == switch and int(hostsLocation[host][1]) == inPort and str(host) == dst:
                                # log.debug("ignoring NF ignoring ignoring ignoring ignoring ignoring ignoring ")
                                return
                    # log.debug("sending packet to host under me without rule")
                    self.send_packet(packet_in.buffer_id, packet_in.data, int(dstPort), inPort,True)
                    return

            if hostsLocation[src][0] == switch :
                # log.debug("ToR s" + switch + " forwarding packet with to host both mine mine mine mine " + " through port " + str(dstPort)+" src "+src+" dst "+dst)
                self.both_mine(packet_in.buffer_id, packet_in.data, int(dstPort), inPort,src,dst)
            else:
                # log.debug("ToR s" + switch + " forwarding packet with VLAN_SPECIAL to host" + " through port " + str(dstPort)+" src "+src+" dst "+dst+" inport"+str(inPort))
                self.send_and_add_rule(packet_in.buffer_id, packet_in.data, int(dstPort), inPort,dst,True)
        else:
            dstSwitch = hostsLocation.get(dst)[0]
            dstPortOut = Tutorial.getWayPort(switch, dstSwitch)
            
            if packet.type == ethernet.VLAN_TYPE:
                # log.debug("ToR s" + switch + " forwarding packet after setting VLAN_SPECIAL to s" + dstSwitch + " through port " + str(dstPortOut)+" src "+src+" dst "+dst)
                self.knownSend(packet_in.buffer_id,packet_in.data,dstPortOut,inPort)
            else:
                log.debug("ToR switch s" + switch + " adding rule to forward packet when dst switch is not mine should see once for src:" + str(src) + " dst:" + str(dst)+" outport "+str(dstPortOut))
                self.dest_Not_Mine(packet_in.buffer_id, packet_in.data, dstPortOut , inPort , src , dst)

    def handle_PacketIn_SpineSwitch(self,packet,packet_in,src,dst,inPort,switch):
        """
        Defines the part of _handle_PacketIn() where the referring switch is a Spine switch.
        """
        ...
        if int(inSwitch) in range(100, 200):
            if dst == "ff:ff:ff:ff:ff:ff":
                ...
                if dstPort is not None :
                    ...
                    self.send_and_add_rule(packet_in.buffer_id, packet_in.data, int(dstPort), inPort,dst)
            ...
            if dst == "ff:ff:ff:ff:ff:ff":
                ...
                if dstPort is None:
                    ...
                    if len(interCores) > 0:
                        core = random.randint(0,len(interCores)-1)
                        dstPort = Discovery.getPort(int(switch),int(interCores[core]))
                if dstPort is not None:
                    log.debug("Spine s" + switch + " forwarding packet from ToR with VLAN_SPECIAL to s" + dstSwitch + " through port " + str(dstPort))
                    self.send_and_add_rule(packet_in.buffer_id, packet_in.data, int(dstPort), inPort,dst)

    def handle_PacketIn_CoreSwitch(self,packet,packet_in,src,dst,inPort,switch):
        """
        Defines the part of _handle_PacketIn() where the referring switch is a Core switch.
        """
        ...
        if dst == "ff:ff:ff:ff:ff:ff":
            ...
            if dstPort is None:
                ...
                if len(interSpines) > 0:
                    spine =random.randint(0,len(interSpines)-1)
                    dstPort = Discovery.getPort(int(switch),int(interSpines[spine]))
            if dstPort is not None:
                log.debug("Core s" + switch + " forwarding packet with VLAN_SPECIAL to s" + dstSwitch + " through port " + str(dstPort)+" src "+src+" dst "+dst)
                self.send_and_add_rule(packet_in.buffer_id, packet_in.data, int(dstPort), inPort, dst)

    def getMyNfPorts(self,switch):
        """
        Returns the ports connected to NFs for the given ToR switch.
        """

        myNFPorts =[]
        for nf in services:
            if services[nf][NF_DPID] ==switch:
                myNFPorts.append(int(services[nf][NF_PORT]))
        return myNFPorts
    ...
    def send_and_add_rule (self, buffer_id, raw_data, out_port, in_port, dstMac,vlan_clear = False):
        """
        Sends a packet out of the specified switch port and adds rule.
        Used in ToR switches for packets that completed the Policy chain, and
        are destined to a host connected to the referring switch.
        """
        ...
        # Add an action to send to the specified port
        msg.match.dl_vlan = VLAN_SPECIAL 
        msg.match.dl_dst = EthAddr(dstMac)
        if vlan_clear:
            action = of.ofp_action_vlan_vid()
            msg.actions.append(action)
    ...
    def dest_Not_Mine (self, buffer_id, raw_data, out_port, in_port,srcMac,dstMac):
        """
        Sets a rule for the ToR switch that matches according to the src and dst
        MAC adresses, for cases when there is no matching policy for the packet.
        """
        ...
        msg.match.dl_dst = EthAddr(dstMac)
        msg.match.dl_src = EthAddr(srcMac)
    ...
    def both_mine (self, buffer_id, raw_data, out_port, in_port,srcMac,dstMac):
        """
        Sends a packet out of the specified switch port.
        If buffer_id is a valid buffer on the switch, use that. Otherwise,
        send the raw data in raw_data.
    ...
    def installNonNFToRRules(switch):
        ...
        for pol in policies:
            ...
            if nfPort is not None:
                action1 = of.ofp_action_output(port = int(nfPort))
                action2 = of.ofp_action_vlan_vid(vlan_vid = VLAN_HOST)
                action3 = of.ofp_action_output(port = of.OFPP_CONTROLLER)
                fm.actions.append(action1)
                fm.actions.append(action2)
                fm.actions.append(action3)
                outPort = str(nfPort) + "  and to Controller: " + str(of.OFPP_CONTROLLER)
                RulesTable.addRule(switch,mList,None,outPort)
                switchesConn[switch].send(fm)
    ...
    def installNFToRRules(switch):
        ...
        for pol in policies:
            ...
            if lastNFSwitch == switch:
                ...
                action2 = of.ofp_action_output(port = of.OFPP_CONTROLLER)
                fm.actions.append(action1)
                fm.actions.append(action2)
        ...
        for pol2 in policies:
            ...
            if services[cList[0]][NF_DPID] != switch:
                ...
                if nfPortOut is not None:
                    fm.match.dl_vlan = of.OFP_VLAN_NONE
                    action1 = of.ofp_action_output(port = int(nfPortOut))
                    action2 = of.ofp_action_vlan_vid(vlan_vid = VLAN_HOST)
                    action3 = of.ofp_action_output(port = of.OFPP_CONTROLLER)
                    fm.actions.append(action1)
                    fm.actions.append(action2)
                    fm.actions.append(action3)
                    outPort = str(nfPortOut) + "  and to Controller: " + str(of.OFPP_CONTROLLER)
                    RulesTable.addRule(switch,mList,None,outPort)
                    switchesConn[switch].send(fm)
                    ...
                    if cIndx == 0 or (cIndx > 0 and services[cList[(cIndx - 1)]][NF_DPID] != switch):
                        ...
                        for p in allPorts:
                            if p < of.OFPP_MAX and p != int(services[c][NF_PORT]):
                                ...
                                fm.actions.append(action1)
                                fm.actions.append(action2)
                                if p not in graphPorts:
                                    action3 = of.ofp_action_vlan_vid(vlan_vid = VLAN_HOST)
                                    action4 = of.ofp_action_output(port = of.OFPP_CONTROLLER)
                                    fm.actions.append(action3)
                                    fm.actions.append(action4)
        ...
        for l in matchList:
            if l[0] == "in_port":
                ...
                if "/" not in l[1]:
                    ...
                    splittedL = l[1].split("/")
                    subnetSrc = compute_Subnet(splittedL[0], splittedL[1])
                    subnetAddr = subnetSrc + "/" + splittedL[1]
                    fm.match.nw_src = subnetAddr
                ...
                if "/" not in l[1]:
                    ...
                    splittedL = l[1].split("/")
                    subnetDst = compute_Subnet(splittedL[0], splittedL[1])
                    subnetAddr = subnetDst + "/" + splittedL[1]
                    fm.match.nw_dst = subnetAddr
    ...
    def getWayPort(switch, nfSwitch):
        ...
        if len(interSpines) > 0 :
            spine =random.randint(0,len(interSpines)-1)
            nfPort = Discovery.getPort(int(switch), int(interSpines[spine]))
            return int(nfPort)
        else:
            nfCores = Discovery.torGetCore(nfSwitch)
            ...
            for s in allUpper:
                if int(s) in range(100, 200) and s in nfCores:
                    ...
                    if len(interCores) > 0 :
                        posibleSpine.append(s)
            out =random.randint(0,len(posibleSpine)-1)
            nfPort = Discovery.getPort(int(switch), int(posibleSpine[out]))
            return int(nfPort)
...
class Discovery:
    ...
    def _handle_ConnectionUp(self, event):
        ...
        if int(str(event.dpid)) in range(300,400):
            Discovery.NFTimers[int(str(event.dpid))] = Timer(Discovery.NF_INTERVAL, self.detectChangeNFLocation, [event], True)
    ...
    def _handle_ConnectionDown(self, event):
        ...
        #stoping NF timer
        if int(str(event.dpid)) in range(300,400):
            Discovery.NFTimers[int(str(event.dpid))].stop()
    ...
    def detectChangeNFLocation(self, event):
        """
        sends ARP requests to the serving hosts in order to determine
        the locations of the NFs.
        """
        ...
        for nf in services:
            r = arp()
            r.opcode = arp.REQUEST
            r.protodst = IPAddr(services[nf][1])
            e = ethernet(type=ethernet.ARP_TYPE, dst=EthAddr("FF:FF:FF:FF:FF:FF"))
            e.set_payload(r)
            msg = of.ofp_packet_out(data = e.pack())
            allPorts = [int(p) for p in Discovery.graph.nodes[int(str(event.dpid))].connection.ports]
            corePorts = Discovery.getPorts(int(str(event.dpid)))
            hostPorts = list(set(allPorts) - set(corePorts))
            for p in hostPorts:
                if p < of.OFPP_MAX:
                    action = of.ofp_action_output(port = p)
...
def launch ():
    ...
    [VLAN_SPECIAL,VLAN_HOST,serv, pol] = parseConfigFile(CONFIG_FILENAME)
